...
class StoredOpenIDAssociation(db.Model):
...
class OpenIDUsedNonce(db.Model):
...
class DatastoreStore(OpenIDStore):
  ...
  def storeAssociation(self, server_url, association):
    ...
    """
    assoc = StoredOpenIDAssociation(url=server_url,
  ...
  def getAssociation(self, server_url, handle=None):
    ...
    """
    query = StoredOpenIDAssociation.all().filter('url', server_url)
    if handle:
  ...
  def removeAssociation(self, server_url, handle):
    ...
    """
    query = StoredOpenIDAssociation.gql('WHERE url = :1 AND handle = :2',
                            server_url, handle)
  ...
  def cleanupNonces(self):
    ...
    @returntype: int
    """
    query = OpenIDUsedNonce.gql('WHERE timestamp < :1', self._expiration_datetime())
  ...
  def cleanupAssociations(self):
    ...
    @returntype: int
    """
    query = StoredOpenIDAssociation.gql('WHERE created < :1', self._expiration_datetime())
    return self._cleanup_batch(query)
