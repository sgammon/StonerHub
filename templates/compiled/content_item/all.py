...
def run(environment):
    name = 'source/content_item/all.html'

    def root(context, environment=environment):
        parent_template = None
        if 0: yield None
        parent_template = environment.get_template('layouts/main-twocolumn.html', 'source/content_item/all.html')
        for name, parent_block in parent_template.blocks.iteritems():
            context.blocks.setdefault(name, []).append(parent_block)
        included_template = environment.get_template('macros/datagrid.html', 'source/content_item/all.html').module
        l_new_datagrid = getattr(included_template, 'new_datagrid', missing)
        if l_new_datagrid is missing:
            l_new_datagrid = environment.undefined("the template %r (imported on line 2 in 'source/content_item/all.html') does not export the requested name 'new_datagrid'" % included_template.__name__, name='new_datagrid')
    ...
    def block_title(context, environment=environment):
        if 0: yield None
        yield u'All content across all repositories'
    ...
    def block_content(context, environment=environment):
        if 0: yield None
        yield u"\n\n<div>\n\t<div id='gridTitle'><h2>Showing: All content across all repositories</h2></div>\n\t<div id='results'>\n\t\tWaiting for results...\n\t</div>\t\n</div>\n\n"

    def block_header(context, environment=environment):
        l_link = context.resolve('link')
        if 0: yield None
        yield u'<h1>All content</h1> <span><a href="%s">List of Repositories</a> <a href="%s">List of Tags</a></span>' % (
    ...
    def block_sidebar_content(context, environment=environment):
        if 0: yield None
        yield u'Every piece of content is placed in a <b>Repository</b>.<br /><br />This view lists all content in the system, regardless of where it is stored.'
